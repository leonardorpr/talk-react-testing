...
describe('[COMPONENTS] TodoItem', () => {
  it('Should render correctly', () => {
    const { container } = render(<TodoItem time='15 minutos' label='Fazer café' done completeTask={jest.fn()} />)
    expect(container).toMatchSnapshot()
  })

  it('Should check if a task is not completed', () => {
    const { container } = render(<TodoItem time='15 minutos' label='Fazer café' done={false} completeTask={jest.fn()} />)
