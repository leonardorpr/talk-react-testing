...
describe('[COMPONENTS] TaskItem', () => {
  it('Should render correctly', () => {
    const { container } = render(<TaskItem time='15 minutos' label='Fazer café' done toggle={jest.fn()} />)
    expect(container).toMatchSnapshot()
  })

  it('Should check if a task is not completed', () => {
    const { container } = render(<TaskItem time='15 minutos' label='Fazer café' done={false} toggle={jest.fn()} />)
